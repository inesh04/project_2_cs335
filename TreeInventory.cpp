...
void Inventory<Comparator, Tree>::queryHelper(const Item& start, const Item& end, const Node* root, std::unordered_set<Item>& result) const
{
    ...
    // nodeAboveMin: current node is >= start (not below the lower bound)
    bool nodeAboveMin = !Comparator::lessThan(root->value_, start);
    // nodeBelowMax: current node is <= end (not above the upper bound)
    bool nodeBelowMax = !Comparator::lessThan(end, root->value_);

    // Left subtree contains only items strictly less than this node.
    // If this node is already >= start, left children may also be in range.
    // If this node < start, all left children are < start too — prune left.
    if (nodeAboveMin) {
        queryHelper(start, end, root->left_, result);
    }

    // Include this node if it falls within [start, end].
    if (nodeAboveMin && nodeBelowMax) {
        result.insert(root->value_);
    }

    // Right subtree contains items >= this node.
    // If this node is already > end, all right children are > end too — prune right.
    if (nodeBelowMax) {
